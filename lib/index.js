...
function createOutputFiles(api) {
    ...
    //======================================================================================
    // Write api_data.json by apiGroup

    var blocks = JSON.parse(api.data);
    var uniqBlocksByGroup = _.sortBy(_.unionBy(blocks,'group'), 'group');

    _.each(uniqBlocksByGroup, function (uniq, i) {
        var subData = _.filter(blocks, function (block) {
            return block.group === uniq.group;
        });

        var subDataString = JSON.stringify(subData);

        app.log.verbose('write js file: ' + app.options.dest + uniq.group +'/'+ 'api_data.js');
        if( ! app.options.simulate) {
            fs.emptyDirSync(app.options.dest + uniq.group);
            fs.writeFileSync(app.options.dest + uniq.group + '/' + './api_data.json', subDataString + '\n');
        }
    });
    //======================================================================================
    ...
    if( ! app.options.simulate) {
        ...
        var navData = _.map(blocks, function (block) {
            return _.pick(block, ['title', 'group', 'name', 'type', 'version', 'url']);
        });

        var navDataString = JSON.stringify(navData);
        fs.writeFileSync(app.options.dest + './api_data.js', 'define({ "api": ' + navDataString + ' });' + '\n');
    }
